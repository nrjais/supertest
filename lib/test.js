...
function Test(app, method, path) {
  ...
  this.bodyAsserts = this.assertCreater('assertBody');
  this.headersAsserts = this.assertCreater('assertHeaders');
  this.cookieAsserts = this.assertCreater('assertCookie');
}
...
Test.prototype.end = function (fn) {
  ...
  var end = Request.prototype.end;
    ...
    if (server && server._handle) return server.close(assertion);
    assertion();
  });
...
Test.prototype.assertBody = function (checker, ...args) {
  return function (res) {
    checker(res.text, ...args);
  }
}

Test.prototype.assertHeaders = function (checker, ...args) {
  return function (res) {
    let key = args.shift();
...
Test.prototype.assertCreater = function (resAssertion) {
  let functions = Object.keys(assert);
  let self = this;
  return functions.reduce((a, fun) => {
    a[fun] = function (...args) {
      let fn = self[resAssertion](assert[fun], ...args);
...
Test.prototype.assertCookie = function (checker, ...args) {
  return function (res) {
    let key = args.shift();
    let headers = getAllKeysInLowerCase(res.headers);
